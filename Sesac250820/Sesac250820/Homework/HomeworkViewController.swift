...
class HomeworkViewController: UIViewController {
    ...
    private let allUsers = BehaviorRelay<[Person]>(value: [])
    private let selectedUsers = BehaviorRelay<[String]>(value: [])
    ...
    override func viewDidLoad() {
        ...
        allUsers.accept(sampleUsers)
    }
     
    private func bind() {
        allUsers
            .bind(to: tableView.rx.items(cellIdentifier: PersonTableViewCell.identifier, cellType: PersonTableViewCell.self)) { [weak self] index, person, cell in
                self?.configureTableCell(cell: cell, person: person)
            }
            .disposed(by: disposeBag)
        
        selectedUsers
            .bind(to: collectionView.rx.items(cellIdentifier: UserCollectionViewCell.identifier, cellType: UserCollectionViewCell.self)) { index, name, cell in
        ...
        tableView.rx.itemSelected
            .bind(with: self) { owner, indexPath in
                ...
                let selectedPerson = owner.allUsers.value[indexPath.row]
                var currentSelected = owner.selectedUsers.value
                if !currentSelected.contains(selectedPerson.name) {
                    currentSelected.append(selectedPerson.name)
                    owner.selectedUsers.accept(currentSelected)
                }
            }
            .disposed(by: disposeBag)
        ...
        searchBar.rx.searchButtonClicked
            ...
            .bind(with: self) { owner, searchText in
                ...
                // allUsers에 추가
                var currentUsers = owner.allUsers.value
                currentUsers.append(newPerson)
                owner.allUsers.accept(currentUsers)
    ...
    private func configureTableCell(cell: PersonTableViewCell, person: Person) {
